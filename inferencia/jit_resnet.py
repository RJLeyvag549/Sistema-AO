...
# ===============================================================
# ResNet-18 JIT helpers (mismo patron que ResNet-10)
# ===============================================================

def trace_resnet18(model: ResNet18, device: torch.device) -> torch.jit.ScriptModule:
    """Traza ResNet-18 en eval mode."""
    model = model.to(device)
    model.eval()
    example = _example_input(device)
    with torch.no_grad():
    ...
    max_diff = (eager_out - jit_out).abs().max().item()
    if max_diff > 1e-4:
        raise RuntimeError(f"TorchScript ResNet18 diverge (max_diff={max_diff:.2e})")
    return traced


def load_resnet18_for_inference(
    weights_path: str,
    device: torch.device,
    *,
    force_recompile: bool = False,
    ...
    """
    Carga ResNet-18 optimizado con TorchScript.
    Reutiliza el artefacto .pt en cache si es mas reciente que los pesos .pth.
    """
    jit_path = jit_path_for_weights(weights_path)
    meta = {
        "backend": "torchscript",
    ...
    if not force_recompile and _jit_cache_is_fresh(jit_path, weights_path):
        t0 = time.perf_counter()
        traced = load_traced_resnet10(jit_path, device)  # carga generica de JIT
        meta["weights_loaded"] = True
        meta["compile_ms"] = (time.perf_counter() - t0) * 1000
        return traced, meta

    model = ResNet18(in_channels=2)
    if os.path.exists(weights_path):
        state = torch.load(weights_path, map_location=device, weights_only=True)
        model.load_state_dict(state)
        meta["weights_loaded"] = True
    ...
    t0 = time.perf_counter()
    traced = trace_resnet18(model, device)
    meta["compiled_at_startup"] = True
    meta["compile_ms"] = (time.perf_counter() - t0) * 1000

    if meta["weights_loaded"]:
        save_traced_resnet10(traced, jit_path)  # guardar es generico

    return traced, meta


def export_resnet18_jit(weights_path: str, device: torch.device | None = None) -> str:
    """Exporta TorchScript de ResNet-18 tras entrenamiento."""
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    _, meta = load_resnet18_for_inference(weights_path, device, force_recompile=True)
    return meta["jit_path"]
